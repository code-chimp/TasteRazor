...
namespace TasteRazor.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
    public class RegisterModel : PageModel
    {
        ...
        {
            var role = Request.Form["rdoRole"].ToString();
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    _roleManager
                        .CreateAsync(new IdentityRole(Constants.CustomerRole))
                        .GetAwaiter()
                        .GetResult();
                    _roleManager
                        .CreateAsync(new IdentityRole(Constants.FrontDeskRole))
                        .GetAwaiter()
                        .GetResult();
                    _roleManager
                        .CreateAsync(new IdentityRole(Constants.KitchenRole))
                        .GetAwaiter()
                        .GetResult();
                    _roleManager
                        .CreateAsync(new IdentityRole(Constants.ManagerRole))
                        .GetAwaiter()
                        .GetResult();
                }
                ...
                if (result.Succeeded)
                {
                    ...
                    else if (role == Constants.KitchenRole)
                    {
                        await _userManager.AddToRoleAsync(user, Constants.KitchenRole);
                    }
                    else if (role == Constants.ManagerRole)
                    {
